...
<div id="wrapper" class="ui-tab">
    ...
    <div id="container">
        <ul class="ui-tab-content">
            ...
            <li>
                ...
                <ul id="mainPage0" class="ui-list ui-border-tb ui-list-active ui-list-cover"
                    style="overflow-x:hidden; overflow-y:auto;height: 91%;">
                </ul>
            </li>
            <!-- 2.活动分类页面-->
            <li id="catalog-wrapper" style="">
                <ul id="catalog-list" class="ui-list ui-border-tb ui-list-active ui-list-cover"
                    style="overflow-x:hidden; overflow-y:auto;height: 91%;">
                </ul>
            </li>
            <!-- 3.活动表单发布 -->
            ...
            <li style="background-color: white;">
                <div class="mycalendar" style="overflow-x:hidden; overflow-y:auto;height: 91%;">
                    <div id="custom-cells" class="datepicker-here" data-language='zh'></div>
                    <ul id="myacty" class="ui-list ui-list-pure ui-border-tb"
    ...
    <!-- 底部的导航栏 -->
    <div id="footer" style="border-top: 1px solid #EEEEEE;">
    ...
    <!-- token的传输的隐藏标签 -->
    <?php echo '<p id="tokentmp" style="display:none;">';
    echo $token;
    echo "</p>" ?>
    <!-- 首页活动标签的隐藏层 -->
    <div class="ui-dialog tagdetail">
        <div class="ui-dialog-cnt" style="background-image: url('../img/bg.png');position:relative;top:-22px;">
            ...
            <div class="ui-dialog-bd" style="padding:0 5px 0 5px">
                ...
                <div>
                    ...
                    <a class="ui-txt-feeds tagticketlink" style="font-size: 15px;" href="#"
                       target="_blank">请点击...</a><br>
            ...
            <div class="ui-dialog-ft ui-btn-group">
                ...
                <button type="button" id="addcalendar">加入日历</button>
            </div>
        </div>
    </div>
    <!-- 我的日历界面删除某个活动时的温馨提示层 -->
...
<script>
    ...
    //主页标签的加载函数,MODE为0首页标签加载,为2管理员页面标签加载
    function getMainPage(MODE, TOKEN) {
        $j.get("actytag.php",
                ...
                if (status == "success") {
                    ...
                    if (data.length != 0) {
                        for (var i = 0; i < data.length; i++) {
                            var actid = parseInt(data[i].id) + parseInt(MODE);
                            $j("#mainPage" + MODE).append("<li class=\"ui-border-t tag\" id=\"" + actid + "\"></li>");
                            $j("#mainPage" + MODE + " li#" + actid).append("<div class=\"ui-list-img\">" + "<span style=\"background-image:url(" + data[i].aimg + ");\"></span>" + "</div>");
                            var date = data[i].astart.replace("2017-", ""),
                                ticket = (data[i].aticket == "ticket" ? "需要抢票" : "无需抢票");
                            $j("#mainPage" + MODE + " li#" + actid).append("<div class=\"ui-list-info\">" + "<h1 class=\"ui-nowrap\">" + data[i].aname + "</h1>" + "<p class=\"ui-nowrap\" style=\"display: block;\">" + date + "&emsp;" + ticket + "</p>" + "</div>");
                        }
                    }
                } else {
                }
    ...
    //标签详情的浮层加载函数
    function getTagDetails(mode, ID, manager=false) {
        if (manager) {
            ID -= 32;
        }
        $j.get("actytag.php",
    ...
    function createActyTags(seed) {
        ...
        for (var i = 0; i < seed.length; i++) {
            var $li = $j("<li class=\"ui-border-t\"></li>");
            ss = seed[i];
            $ul.append($li);
            var tmpyear = new Date().getFullYear();
            var end = ss.aend.replace(tmpyear + "-", "").replace('-', '/').replace(/ /g, '/');
            var acty_start = ss.astart.replace(/-/g, '/').replace(' ', '/');
            var $h2 = $j("<div class=\"myacty-nav\"><h2 class=\"myacty-title\" style='width: 70px;display: inline-block;'>" + ss.aname + "</h2>" + "<span class=\"myacty-time\" style=\"font-size:14px; text-align:center;\">" + acty_start + " — " + end + "</span></div>");
            $li.append($h2);
            ...
            var $h41 = $j("<h3 class=\"myacty-score\">德育分:</h3>");
            $div.append($h41);
            if (ss.ascore != "0.00") {
        ...
        $ul.append("<button class=\"ui-btn-lg backbtn\" style=\"display:none;\">返回</button>");
        if ($j(".datepicker-here").css("display") == "none") {
            $j(".backbtn").show();
            $j(".myacty-nav").css("color", "#ff9a19");
        }
        $j(".myacty-nav").click(function () {
            $j(".myacty-nav").css("color", "#ff9a19");
            $j(this).next().slideToggle("slow");
            $j('.datepicker-here').slideUp();
            $j('.backbtn').show();
        });
        $j('.backbtn').click(function () {
            $j(".myacty-nav").css("color", "black");
            $j(".myacty-nav").next().hide();
            $j('.datepicker-here').slideDown();
            $(this).hide();
        });
    }
    ...
    window.addEventListener('load', function () {
        ...
        getMainPage(0, '');
        //1.1主页标签详情的浮层加载
        $j("#mainPage0").on("tap", ".tag", function () {
            var id = $(this).attr('id');
            getTagDetails(1, id);
        });
        //1.2用户选择某个活动加入数据库,重复加入返回提示
        $('#addcalendar').click(function () {
            if ($('#addcalendar').html() == '加入日历') {
                ...
                function (data, status) {
                    if (status == 'success') {
                        ...
                        for (var i = 0; i < acty.length; i++) {
                            ...
                            $li2.append("<div class=\"ui-list-info\"></div>");
                            var tmpyear = new Date().getFullYear();
                            //var end = acty[i].aend.replace( tmpyear+"-","" ).replace(' ','/').replace('-','/');
                            var start = acty[i].astart.replace(tmpyear + "-", "").replace(' ', '/').replace('-', '/');
                            var date = start;
                            if (acty[i].aticket == "ticket")
                                ...
                                $li2.children("div .ui-list-info").append("<h1 class=\"ui-nowrap\">" + acty[i].aname + "</h1>").append("<p class=\"ui-nowrap\" style=\"display: block;\">" + date + "&emsp;无需抢票 </p>");
                        }
                    }
                });
        });
        ...
        $("#calendar").click(function () {
            var date = new Date();
            var year = date.getFullYear(), month = date.getMonth() + 1, currentDate = date.getDate(),
                daytmp = date.getDate();
            if (month < 10) {
                month = "0" + month;
            }
                ...
                function (data) {
                    ...
                    $picker.datepicker({
                        ...
                        onRenderCell: function (date, cellType) {
                            var year1 = date.getFullYear(), month1 = date.getMonth() + 1, daytmp1 = date.getDate();
                            if (month1 < 10) {
                                month1 = "0" + month1;
                            }
                            if (daytmp1 < 10) {
                            ...
                            var current = "" + year1 + month1 + daytmp1;
                            //对有活动的日期加上小点
                            if (cellType == 'day' && eventDates.indexOf(current) != -1) {
                                return {
        ...
        $j("#myacty").on("tap", ".deletebtn", function () {
            ...
            // var deltimes=1;
            // console.log(id);
            // $("#deleteno").tap(function (){deltimes--; console.log('no'); });
            // $("#deleteyes").tap(function () {
            //   console.log(deltimes);
            //if(deltimes != 1){return;}
            id = id.replace("d", "");
            $j.get("actytag.php", {"actyid": id, "mode": "42", "token": $('#tokentmp').html()},
                function (data, status) {
                    if (status == "success") {
                        btn.html(data);
                        setTimeout(function () {
                            btn.parents(".ui-border-t").remove();
                            if ($("#myacty li").length == 0) {
                                $(".backbtn").click();
                                $("#calendar").click();
                            }
                        }, 600);
